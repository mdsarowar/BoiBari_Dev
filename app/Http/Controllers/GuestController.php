...
//        return $request;
    ...
    public function showpage($slug)
    {
        ...
        if ($page) {
            return view('frontend.singlepage', compact('conversion_rate', 'page'));
    ...
    public function faq()
    {
        ...
        $faqs = Faq::where('status', '1')->orderBy('id', 'desc')
            ->paginate(10);
        return view('frontend.faq', compact('conversion_rate', 'faqs'));
    ...
    public function choose_dist(Request $request)
    {
        $div_id = $request['div_id'];
        $dist=Districts::where('division_id',$div_id)->get();
        return response()->json($dist);
    }
     public function choose_upazila(Request $request)
    {
        $dist_id = $request['dist_id'];
        $upazila=Upazilas::where('district_id',$dist_id)->get();
        return response()->json($upazila);
    }
    public function choose_union(Request $request)
    {
        $upa_id = $request['upa_id'];
        $union=Unions::where('upazilla_id',$upa_id)->get();
