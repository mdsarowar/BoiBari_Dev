...
//        return $number;
        ...
        $neworder->manual_payment = $number ? '1' : '0' ;
        $neworder->manual_payment_number = $number;
        ...
        foreach ($cart_table2 as $key => $invcart) {
            ...
            if (isset($invcart->variant)) {
                ...
                if ($invcart->semi_total != 0) {
                    ...
                    if ($findvariant->products->tax_r != '') {
                        ...
                        $vp = $p + $taxrate_db;
                        $tam = intval($findvariant->products->offer_price) / intval($vp) * intval($taxrate_db);
                        $tam = sprintf("%.2f", $tam);
            ...
            if ($invcart->product && $invcart->product->vender?$invcart->product->vender->role_id:'' == 'v') {
...
//            }
        ...
        foreach ($cart_table as $carts) {
            ...
            if (isset($carts->simple_product)) {
                ...
                $qty= $carts->simple_product->qty + $carts->qty;
                DB::table('simple_products')->where('id', $carts->simple_product->id)->update(['stock' => $used,'qty'=>$qty]);
