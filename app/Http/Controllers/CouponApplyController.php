...
class CouponApplyController extends Controller
{
    public function apply(Request $request)
    {
        ...
        $cpn = Coupan::where('code', '=', $request->coupon)->first();
        ...
        if(!isset($cpn)){
            if (!Auth::check())
            {
                ...
                Cart::where('user_id', Auth::user()->id)->update(['distype' => NULL, 'disamount' => NULL,'coupan_id' => NULL]);
                notify()->error(__('Invalid Coupan !'));
                return back();
            }
        ...
        if (isset($cpn))
        {
            ...
            if (date('Y-m-d', strtotime($cpn->expirydate)) >= $today)
            {
                ...
                else
                {
                    ...
                    Cart::where('user_id', Auth::user()->id)->update(['distype' => NULL, 'disamount' => NULL,'coupan_id' => NULL]);
                    notify()->error(__('Coupan code max usage limit reached !'));
                    return back();
                }
            ...
            else
            {
                Session::forget('coupanapplied');
                Cart::where('user_id', Auth::user()->id)->update(['distype' => NULL, 'disamount' => NULL, 'coupan_id' => NULL]);
                notify()->error(__('Coupan Expired'));
                return back();
            }
        ...
        else
        {
            Session::forget('coupanapplied');
            Cart::where('user_id', Auth::user()->id)->update(['distype' => NULL, 'disamount' => NULL, 'coupan_id' => NULL]);
            notify()->error(__('Coupan code is invalid'));
            return back();
        }
