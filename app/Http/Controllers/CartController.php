...
//
//        }
//        Session::forget('total','guest_cart');
//        return Session::all();
...
//                    $stock = AddSubVariant::withTrashed()->find($row['variantid']);
                    ...
                    if ($product) {
                        ...
                        if (empty($product->offer_price)) {
                            $stotal = $qty * $product->price;
//                            $product->mrp = $qty * $product->price;
                        } else {
                            $stotal = $qty * $product->offer_price;
...
//        return Session::get('cart');
        ...
        return view("frontend.cart", compact('cart_table', 'conversion_rate', 'oot','product_data'));
    ...
    public function add_item($id, $variantid, $varprice, $varofferprice, $qty)
    {
        ...
        if (!empty($user)) {
            ...
            if ($cart_table) {
                ...
                if ($cart_table->variant_id == $variantid) {
                    ...
                    if ($tqty <= $limit) {
                        ...
                        notify()->success(__('Product Quantity updated in cart !'));
                        return redirect('cart');
                        

                    } else {

                        notify()->warning(__('Product already in cart with max quantity limit !'));
                        return redirect('cart');
            ...
            notify()->success(__('Product added in cart !'));
            return redirect('cart');
        } else {
            ...
            if (!empty(Session::get('cart'))) {
                ...
                if ($avbl == 1) {
                    ...
                    if ($tqty <= $limit) {
                        ...
                        $data['msg'] = 'Product Quantity updated in cart !';
                        $data['status'] = 'success';
                       if($request->data_from=='Checkout'){
                        ...
                        notify()->error(__('Product already in cart with max quantity limit !'));
                        return redirect('cart');
                    }
    ...
    public function add_product_card(Request $request ,$id, $variantid, $varprice, $varofferprice, $qty)
    {
        $getvenderid = Product::where('id', $id)->first()->vender_id;
            ...
            if ($cart_table) {
                ...
                if ($cart_table->variant_id == $variantid) {
                    ...
                    if ($tqty <= $limit) {
                        ...
                        $data['msg'] = 'Product already in cart with max quantity limit !';
                        $data['status'] = 'success';
                        $data['cart_count'] = Cart::where('user_id', Auth::user()->id)->count();
                        return $data;
...
//                $product = SimpleProduct::where('id', $ca->pro_id)
//                    ->first();
//                $pro_id = $product->id;
//
...
//                return $pro_id;
            ...
            foreach ($cart as $key => $value) {
                ...
                if ($value['id'] == $id) {
                    $cart[$key]['quantity'] = $days;
                    $cart[$key]['id'] = $id;
                    $product = SimpleProduct::find($id);
//                    return $product;

                    if (empty($product->offer_price)) {
                        $singletotal =  $cart[$key]['quantity'] * $product->price;
//                        return $singletotal;
                    } else {
                        $noffertotal =  $cart[$key]['quantity'] * $product->offer_price;
//                        return $noffertotal;
                    }
...
//                if ($value['id'] == $id) {
                    $product = SimpleProduct::find($value['id']);
//                    if ($product) {

                        if (empty($product->offer_price)) {
                            $sntotal = $value['quantity'] * $product->price;
//                            $product->mrp = $qty * $product->price;
                        } else {
                            $sntotal = $value['quantity'] * $product->offer_price;
//                            $offtotal=$sntotal;
//                            $product->new_offerprice = $qty * $product->offer_price;
//                            $product->mrp = $qty * $product->price;
                        }
...
//        session()->forget('cart');
        ...
        if (!auth()->check()) {
            ...
            if (isset($cart)) {
                ...
                foreach ($cart as $key => $p) {
                    $pro_id = SimpleProduct::where('id', $post)->first();

                    if ($pro_id->stock <= $p['quantity']) {
                        notify()->error(__('Product stock limit reached !'));
                        $data['msg'] = 'Product stock limit reached !';
                        $data['status'] = 'success';
                        return $data;
//                        return back()->with("failure", __("Stock is not available"));
                    }
                    if ($p['quantity'] > $pro_id->max_order_qty) {

                        notify()->error(__('Product max qty limit reached !'));
                        $data['msg'] = 'Product max qty limit reached !';
                        $data['status'] = 'success';
//                        $data['cart_count'] = Cart::where('user_id',auth()->id())->count();
            ...
            $product_data = array();
            $product_data['id'] = $post;
            $product_data['quantity'] = $qty + 1;

            $cart[] = $product_data;

            Session::put('cart', $cart);
            $cartc=count(Session::get('cart'));
            notify()->success(__('added in cart for guest user'));
            $data['msg'] = 'Product added in cart !';
            $data['cartc'] = $cartc;
            $data['status'] = 'success';
            return $data;
...
//        $cart->vender_id = $product->store->user->id;
    ...
    public function addsimpeproductincart(Request $request, $pro_id, $price, $offerprice)
    {
//        return 'hel';
...
//                foreach (Session::get('guest_cart') as $cart){
//                    if ($cart == $pro_id){
////                        notify()->success(__('Your Product already in cart !!'));
//                        $data['msg'] = 'Your Product already in cart !!';
//                        $data['status'] = 'success';
//                        return redirect('cart');
//                    }
//                }
//                Session::push('guest_cart', $pro_id);
//                $count=Session::get('guest_cart_count')+1;
//                Session::put('guest_cart_count',$count);
//
//            }else{
//                Session::put('guest_cart',[$pro_id]);
//                Session::put('guest_cart_count',1);
//            }
//            notify()->success(__('added in cart for guest user'));
//            $data['msg'] = 'Product added in cart !';
//            $data['status'] = 'success';
