	...
	<div class="card page_border">
		...
		<div class="card-header p-4">
			...
			<div class="mt-4 float-right">
				...
				<hr>
				{{__("Date:")}} <b>{{$design?date($design->date_format,strtotime($getInvoice->created_at)):'' }}</b>
				<br>
...
{{--				</div>--}}
				<div class="col-sm-4 ">
					...
					<div>
						{{ $address->division_id ? $address->getDivisions->bn_name.',' : '' }}
						{{ $address->district_id ? $address->getdistrict->bn_name.',' : '' }}
						{{ $address->upazila_id ? $address->getupazila->bn_name.',' : '' }}
						{{ $address->union_id ? $address->getunion->bn_name.',' : '' }}
...
{{--					<div>{{ $div->bn_name }},{{ $dist->bn_name }},{{ $upa->bn_name }}</div>--}}
...
<script>
	function printit(){
		@if($design && $design->print_mode == 'landscape')
