...
@section('title',__('All Publishers'))
@section('body')
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
                <div class="card-header">
                  ...
                  <div class="row">
                    <div class="col-lg-6">
                      <h5 class="card-title"> {{__("All Publisher")}}</h5>
                    </div>
                    <div class="col-md-6">
                      <div class="widgetbar">

{{--                        <a href="{{url('admin/category/create')}} " class="btn btn-primary-rgba mr-2">--}}
                        <a href="{{route('publisher.create')}} " class="btn btn-primary-rgba mr-2">
                            <i class="feather icon-plus mr-2"></i> {{__("Add Publisher")}}
                        </a>
...
{{--                        </div>--}}
                ...
                <table id="full_detail_table" class="cattable w-100 table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr class="row1" data-id="{{$pub->id}}">
                      
                      <td>{{ ++$key }}</td>

                      <td>
                        @if($pub->image != '' && file_exists(public_path().'/images/Publishers/'.$pub->image) )
                        <img class="pro-img mr-2" align="left" src="{{asset('images/Publishers/'.$pub->image)}}" title="{{ $pub->title }}">
{{--                        <img class="pro-img mr-2" align="left" src="{{url('images/category/'.$pub->image)}}" title="{{ $pub->title }}">--}}
                        @else
                        <img class="pro-img mr-2" align="left" title="{{ $pub->title }}" src="{{ Avatar::create($pub->title)->toBase64() }}" />
                        @endif
                      </td>
                      ...
                      <td>
                        ...
                        <p><b>{{__('Name')}}: </b><span class="font-weight500">{{$pub->title}}</span></p>
                        <p><b>{{__('Description')}}: </b><span class="text-justify font-weight500">{{substr(strip_tags($pub->description), 0, 100)}}{{strlen(strip_tags(
                           $pub->description))>100 ? '...' : ""}}</span></p>
                      </td>
            
                      <td>
                        @can('category.edit')
                        <form method="POST" action="{{ route('cat.quick.update',$pub->id) }}">
                          {{ csrf_field() }}
                          <button @if(env('DEMO_LOCK')==0) type="submit" @else title="{{ __("This operation is disabled in Demo !") }}"
                            disabled="" @endif class="btn btn-sm btn-rounded {{ $pub->status ==1 ? 'btn-success-rgba' : 'btn-danger-rgba' }}">
                            {{ $pub->status==1 ? __('Active') : __('Deactive') }}
                          </button>
                        </form>
                        @endcan
                      </td>
                      <td>
                        @can('category.edit')
                        <form method="POST" action="{{ route('cat.featured.quick.update',$pub->id) }}">
                          {{ csrf_field() }}
                          <button @if(env('DEMO_LOCK')==0) type="submit" @else title="{{ __("This operation is disabled in Demo !") }}"
                            disabled="" @endif class="btn btn-sm btn-rounded {{ $pub->featured ==1 ? 'btn-success-rgba' : 'btn-danger-rgba' }}">
                            {{ $pub->featured==1 ? 'Yes' : 'No' }}
                          </button>
                        </form>
                        @endcan
                      </td>
...
{{--                            @can('category.edit')--}}
                              <a class="dropdown-item" href="{{route('publisher.edit',$pub->id)}}"><i class="feather icon-edit mr-2"></i>{{ __("Edit") }}</a>
{{--                              @endcan--}}
          
                              @can('category.delete')
                              <a class="dropdown-item btn btn-link" data-toggle="modal" data-target="#delete{{ $pub->id }}" >
                                <i class="feather icon-delete mr-2"></i>{{ __("Delete") }}</a>
                            </a>
                              @endcan
                          </div>
                      </div>
                      <div class="modal fade bd-example-modal-sm" id="delete{{$pub->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-sm">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <form method="post" action="{{route('publisher.destroy',$pub->id)}}" class="pull-right">
                                        {{csrf_field()}}
                                        {{method_field("DELETE")}}
