...
<?php
  $data['heading'] = 'Return Orders';
  $data['title0'] = 'Order & Invoices';
  $data['title1'] = 'Return Orders';
?>
                    ...
                    <table class="font-size-14 width100 table table-striped">
                        ...
                        <tbody>
                            <tr>
                                    ...
                                    <div class="col-md-11">
                                        @if(isset($order->getorder->variant))
                                            @if($order->getorder->variant->variantimages)
                                                <img width="100px" class="mr-2 float-left object-fit" src="{{url('variantimages/'.$order->getorder->variant->variantimages->mainimage)}}" />
                                            @else
                                                <img width="100px" class="mr-2 float-left object-fit" src="{{ Avatar::create($order->getorder->variant->products->name) }}" />
                                            @endif
                                        @endif
                                ...
                                <td><b><i class="{{ $order->mainOrder?$order->mainOrder->paid_in:'' }}"></i>{{ $order->amount }} </b><br>
                                ...
                                <td>
                                    ...
                                    <p><b>{{ __("Transcation FEE:") }}</b> &nbsp;<i
                                            class="{{ $order->mainOrder?$order->mainOrder->paid_in:'' }}"></i>
                                            {{ $order->txn_fee }} ({{__("During Bank Transfer")}})</p>
                                    @endif
            
                                    @if($order->getorder->variant)
                                        @if($order->variant->products->returnPolicy && $order->variant->products->returnPolicy->amount !=0 || $order->variant->products->returnPolicy->amount
                                        !='')
                                    ...
                                    @if(isset($order->getorder->simple_product))
                                        @if(isset($order->getorder->simple_product->returnPolicy))
